...
The ElPiGraph package contains a number of functions to derive the pseudotime associted with each point. This is particularly relevant in biological contexts. 
...
Since the paths derived are unique, the node 2 could be either at the beginning or at the end of teh path considered. therefore, we select all the path starting or ending in 2 
...
Then, we will compute a projection structure, which contains inforlation relative to the projection of points on the edge of the graph
...
We are now able to obtain the pseudotime, via the `getPseudotime` function. We will use `lapply` to compute all of the pseudotime at once. Moreover, the functions requires nodes to be passed as a vector os strings, so we will need to use the `names` function to convert the sequence of vertices.
